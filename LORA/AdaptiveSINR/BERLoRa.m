...
sigma_i = sigma * 10^(INR/20);
interference = sigma_i/sqrt(2) *(randn(M*Ns,1) + 1j*randn(M*Ns,1));
...
for CtrSNR = 1 : length(SNR)
    ...
    Rx1 = signal + n + interference;
    Rx2 = h.*signal + n + interference;
    Ps = mean(abs(signal).^2);
    Pn = mean(abs(n).^2);
    Pi = mean(abs(interference).^2);

    SINR(CtrSNR) = 10*log10(Ps/(Pn+Pi));
    %% Non-Coherent
    % AWGN
